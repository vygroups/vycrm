            ...
            <div class="card-grid stats-panel" id="statsPanel" style="display: block; width: 100%; margin-bottom: 40px;">
                ...
                foreach ($dashboardStats as $moduleKey => $stats): ?>
                    <?php if (strpos($moduleKey, 'dyn_') === 0): 
                        ...
                        $totalVal = (int)($stats[0]['value'] ?? 0);
                        $todayVal = (int)($chartData['today'] ?? 0);
                        $weekVal = (int)($chartData['week'] ?? 0);
                        $monthVal = (int)($chartData['month'] ?? 0);
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 20px;">
                    <?php foreach ($widgets as $w): 
                        ...
                        $totalModCount = (int)$totalModStmt->fetchColumn();
                                                                          ...
                                                                          $displayType = $w['display_type'] ?? 'count';
                                                                              ...
                                                                              <div style="display: flex; justify-content: space-between; align-items: center;">
                                                                                  <div style="flex: 1; min-width: 0;">
                                                                                      ...
                                                                                      <div style="display: flex; align-items: baseline; gap: 8px; flex-wrap: wrap;">
                                                                                          <?php if ($displayType === 'percentage'): ?>
                                                                                              <div style="font-size: 32px; font-weight: 800; color: var(--text-main); line-height: 1;">
                                                                                                  <?= $wPercent ?>%
                                                                                              </div>
                                                                                              <span style="font-size: 12px; color: var(--text-muted); font-weight: 600;">(<?= number_format($wCount) ?> / <?= number_format($totalModCount) ?>)</span>
                                                                                          <?php elseif ($displayType === 'both'): ?>
                                                                                              <div style="font-size: 32px; font-weight: 800; color: var(--text-main); line-height: 1;">
                                                                                                  <?= number_format($wCount) ?>
                                                                                              </div>
                                                                                              <span style="display: inline-flex; align-items: center; background: <?= $cardColor ?>18; color: <?= $cardColor ?>; padding: 3px 8px; border-radius: 20px; font-size: 12px; font-weight: 800; border: 1px solid <?= $cardColor ?>30;">
                                                                                                  <?= $wPercent ?>%
                                                                                              </span>
                                                                                          <?php else: ?>
                                                                                              <div style="font-size: 32px; font-weight: 800; color: var(--text-main); line-height: 1;">
                                                                                              ...
                                                                                              <span style="font-size: 12px; color: var(--text-muted); font-weight: 600;">of <?= number_format($totalModCount) ?> total</span>
                                                                                          <?php endif; ?>
                                                                                      </div>
                                                                                  </div>
